...
vec3 getNovaCircle(vec3 tt) {
 if(spell_available){

    float shape = getShape(spell_position, spell_range);

    vec2 uvpoint = spell_position / size;
    vec4 texture = texture2DSRGB(spell_texture, (uv_cord - uvpoint - spell_uv_offset_vec) * vec2(spell_texture_scale));

    vec3 tt = mix(tt, texture.rgb, shape * texture.a * spell_opacity);
    return tt;
 }
    return tt;
}

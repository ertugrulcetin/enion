...
* [`src/enion_cljs/styles.cljs`](src/enion_cljs/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/enion_cljs/`](src/enion_cljs/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/enion_cljs/core.cljs): contains the SPA entry point, `init`
* [`test/enion_cljs/`](test/enion_cljs/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `enion-cljs` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/enion_cljs/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns enion-cljs.example
  (:require [enion-cljs.config :as config])
